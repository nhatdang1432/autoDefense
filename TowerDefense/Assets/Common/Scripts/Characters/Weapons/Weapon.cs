...
namespace MoreMountains.TopDownEngine
{
    ...
    public class Weapon : MMMonoBehaviour 
    {
        ...
        public virtual void SetOwners(Character newOwner, CharacterSkill handleWeapon)
        {
            Owner = newOwner;
            if (Owner != null)
            {
                CharacterSkill = handleWeapon;
                _characterMovement = Owner.GetComponent<Character>()?.FindAbility<CharacterMovement>();
                _controller = Owner.GetComponent<TopDownController>();

                if (CharacterSkill.AutomaticallyBindAnimator)
                {
                    if (CharacterSkill.CharacterAnimator != null)
                    {
                        _ownerAnimator = CharacterSkill.CharacterAnimator;
                    }
                    if (_ownerAnimator == null)
                    {
                        _ownerAnimator = CharacterSkill.gameObject.GetComponentInParent<Character>().CharacterAnimator;
                    }
                    if (_ownerAnimator == null)
                    {
                        _ownerAnimator = CharacterSkill.gameObject.GetComponentInParent<Animator>();
                    }
                }
            }
        }
